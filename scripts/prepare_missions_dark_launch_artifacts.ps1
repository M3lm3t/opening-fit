...
$stages = @(
  @{Name='001a'; From=0; To=119; Requires="perform 1;"; Assert="if to_regclass('public.openingfit_missions') is null or to_regclass('public.openingfit_mission_training_attempts') is null or to_regclass('public.openingfit_mission_encounters') is null or to_regclass('public.openingfit_mission_status_events') is null or (select count(*) from pg_indexes where schemaname='public' and indexname like 'openingfit_mission%')<>8 then raise exception '001A postcondition failed'; end if;"},
