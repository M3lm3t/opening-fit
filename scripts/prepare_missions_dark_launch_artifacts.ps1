...
$expected = @{
  ...
  '004'='6357D09A658F31F7C376001499253A634D778F18FBC4014EA45DB7A1AFA1D020'
}
$sources = @{
...
# SQL Editor-safe split of 003. RPC grant statements remain in their original
# source order in 003C; 003B adds temporary deny-all containment until then.
$source003=Join-Path $root 'supabase/migrations/202608310003_openingfit_mission_training.sql'
$lines003=[IO.File]::ReadAllLines($source003)
if($lines003.Count -ne 172){throw 'Migration 003 line contract changed'}
$stage003AContract="to_regclass('public.openingfit_mission_training_sessions') is not null and (select relrowsecurity from pg_class where oid=to_regclass('public.openingfit_mission_training_sessions')) and exists(select 1 from pg_constraint where conrelid=to_regclass('public.openingfit_mission_training_attempts') and conname='openingfit_attempt_session_owner_fk') and exists(select 1 from pg_policies where schemaname='public' and tablename='openingfit_mission_training_sessions' and policyname='openingfit_mission_training_sessions_select_own' and cmd='SELECT' and roles=array['authenticated']::name[]) and exists(select 1 from pg_trigger where tgrelid=to_regclass('public.openingfit_mission_training_sessions') and tgname='openingfit_protect_training_session' and not tgisinternal) and not has_table_privilege(0,'public.openingfit_mission_training_sessions','select,insert,update,delete') and not has_table_privilege('anon','public.openingfit_mission_training_sessions','select,insert,update,delete') and has_table_privilege('authenticated','public.openingfit_mission_training_sessions','select') and not has_table_privilege('authenticated','public.openingfit_mission_training_sessions','insert,update,delete') and has_table_privilege('service_role','public.openingfit_mission_training_sessions','select,insert,update') and not has_table_privilege('service_role','public.openingfit_mission_training_sessions','delete,truncate,references,trigger')"
$contained003BContract="(select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname=any(array['start_openingfit_mission_training_session','record_openingfit_mission_training_attempt']))=2 and exists(select 1 from pg_proc where oid=to_regprocedure('public.start_openingfit_mission_training_session(uuid,uuid,text,text,jsonb,integer,integer)') and prorettype=to_regtype('jsonb') and prosecdef and proconfig @> array['search_path=public']) and exists(select 1 from pg_proc where oid=to_regprocedure('public.record_openingfit_mission_training_attempt(uuid,uuid,uuid,text,text,text,text,integer,integer,timestamptz,jsonb)') and prorettype=to_regtype('public.openingfit_mission_training_attempts') and prosecdef and proconfig @> array['search_path=public']) and not exists(select 1 from (values(0::oid),('anon'::regrole::oid),('authenticated'::regrole::oid),('service_role'::regrole::oid)) r(oid) cross join (values(to_regprocedure('public.start_openingfit_mission_training_session(uuid,uuid,text,text,jsonb,integer,integer)')), (to_regprocedure('public.record_openingfit_mission_training_attempt(uuid,uuid,uuid,text,text,text,text,integer,integer,timestamptz,jsonb)'))) f(oid) where has_function_privilege(r.oid,f.oid,'execute')) and to_regprocedure('public.complete_openingfit_mission_training_session(uuid,uuid,uuid,text)') is null and public.openingfit_missions_schema_readiness() is not distinct from jsonb_build_object('ready',true,'schemaVersion',1)"
$split003=@(
 @{Name='003a';From=0;To=64;Requires="if to_regprocedure('public.openingfit_missions_schema_readiness()') is null or public.openingfit_missions_schema_readiness() is distinct from jsonb_build_object('ready',true,'schemaVersion',1) then raise exception 'completed migrations 001 and 002 are required'; end if;";Assert="if to_regclass('public.openingfit_mission_training_sessions') is null or not (select relrowsecurity from pg_class where oid=to_regclass('public.openingfit_mission_training_sessions')) or not exists(select 1 from pg_constraint where conrelid=to_regclass('public.openingfit_mission_training_attempts') and conname='openingfit_attempt_session_owner_fk') or not exists(select 1 from pg_trigger where tgrelid=to_regclass('public.openingfit_mission_training_sessions') and tgname='openingfit_protect_training_session' and not tgisinternal) or has_table_privilege('anon','public.openingfit_mission_training_sessions','select') or has_table_privilege('authenticated','public.openingfit_mission_training_sessions','insert') or not has_table_privilege('service_role','public.openingfit_mission_training_sessions','select,insert,update') then raise exception '003A postcondition failed'; end if;"},
 @{Name='003b';From=65;To=116;Requires="if not ($stage003AContract) then raise exception '003A is required'; end if;";After="revoke all on function public.start_openingfit_mission_training_session(uuid,uuid,text,text,jsonb,integer,integer), public.record_openingfit_mission_training_attempt(uuid,uuid,uuid,text,text,text,text,integer,integer,timestamptz,jsonb) from public,anon,authenticated,service_role;`n";Assert="if not ($contained003BContract) then raise exception '003B postcondition failed'; end if;"},
 @{Name='003c';From=117;To=171;Requires="if not ($stage003AContract and $contained003BContract) then raise exception 'contained 003B is required'; end if;";Assert="if to_regprocedure('public.complete_openingfit_mission_training_session(uuid,uuid,uuid,text)') is null or public.openingfit_missions_schema_readiness()->>'schemaVersion'<>'3' or public.openingfit_missions_schema_readiness()->>'trainingReady'<>'true' or has_function_privilege(0,to_regprocedure('public.complete_openingfit_mission_training_session(uuid,uuid,uuid,text)'),'execute') or has_function_privilege('authenticated',to_regprocedure('public.complete_openingfit_mission_training_session(uuid,uuid,uuid,text)'),'execute') or not has_function_privilege('service_role',to_regprocedure('public.complete_openingfit_mission_training_session(uuid,uuid,uuid,text)'),'execute') then raise exception '003C postcondition failed'; end if;"}
)
foreach($stage in $split003){
 $body=($lines003[$stage.From..$stage.To]-join "`n")+"`n"
 $header="-- PRODUCTION WARNING: target $project; SQL Editor split stage $($stage.Name.ToUpper()).`n-- Missions must remain disabled. Never rerun migrations 001 or 002.`nBEGIN;`nDO `$precondition`$ begin $($stage.Requires) end `$precondition`$;`n-- SOURCE MIGRATION 003 STAGE BEGIN`n"
 $tail="-- SOURCE MIGRATION 003 STAGE END`n$($stage.After)DO `$assert`$ begin $($stage.Assert) end `$assert`$;`nCOMMIT;"
 Write-Utf8 (Join-Path $out "openingfit-missions-production-$($stage.Name)-execute.sql") ($header+$body+$tail)
}
$containmentExecute=@"
...
'@
Write-Utf8 (Join-Path $out 'openingfit-missions-production-003a-verification.sql') $verify003A
$verify003B=@"
-- READ ONLY. Migration 003B verification. Run each SELECT independently.
select p.proname,pg_get_function_identity_arguments(p.oid),p.prosecdef,p.proconfig,has_function_privilege(0,p.oid,'execute') public_execute,has_function_privilege('anon',p.oid,'execute') anon_execute,has_function_privilege('authenticated',p.oid,'execute') authenticated_execute,has_function_privilege('service_role',p.oid,'execute') service_execute from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('start_openingfit_mission_training_session','record_openingfit_mission_training_attempt') order by 1,2;
select case when to_regclass('public.openingfit_mission_training_sessions') is null then 'stage_absent' when $stage003AContract and $contained003BContract then 'stage_complete' else 'stage_partial' end classification;
"@
Write-Utf8 (Join-Path $out 'openingfit-missions-production-003b-verification.sql') $verify003B
$verify003C=@'
...
'@
Write-Utf8 (Join-Path $out 'openingfit-missions-production-003c-verification.sql') $verify003C
$source004=Join-Path $root 'supabase/migrations/202608310004_openingfit_missions_rollout.sql'
$lines004=[IO.File]::ReadAllLines($source004)
if($lines004.Count -ne 169){throw 'Migration 004 line contract changed'}
$stage004A="to_regclass('public.openingfit_mission_events') is not null and to_regclass('public.openingfit_mission_activity_outbox') is not null and to_regclass('public.openingfit_mission_allowances') is not null and to_regclass('public.openingfit_mission_notification_candidates') is not null and (select count(*) from pg_class where oid=any(array[to_regclass('public.openingfit_mission_events'),to_regclass('public.openingfit_mission_activity_outbox'),to_regclass('public.openingfit_mission_allowances'),to_regclass('public.openingfit_mission_notification_candidates')]) and relrowsecurity)=4 and not has_table_privilege(0,'public.openingfit_mission_events','select,insert,update,delete') and not has_table_privilege('anon','public.openingfit_mission_events','select,insert,update,delete') and has_table_privilege('authenticated','public.openingfit_mission_events','select') and not has_table_privilege('authenticated','public.openingfit_mission_events','insert,update,delete') and has_table_privilege('service_role','public.openingfit_mission_events','select,insert,update') and not has_table_privilege('service_role','public.openingfit_mission_events','delete,truncate,references,trigger') and exists(select 1 from information_schema.columns where table_schema='public' and table_name='notification_preferences' and column_name='mission_reminders' and is_nullable='NO' and column_default='false'::text)"
$stage004B="to_regprocedure('public.record_openingfit_mission_event(uuid,uuid,text,text,jsonb)') is not null and not exists(select 1 from (values(0::oid),('anon'::regrole::oid),('authenticated'::regrole::oid),('service_role'::regrole::oid))r(oid) where has_function_privilege(r.oid,to_regprocedure('public.record_openingfit_mission_event(uuid,uuid,text,text,jsonb)'),'execute'))"
$stage004C="to_regprocedure('public.complete_openingfit_mission_training_session(uuid,uuid,uuid,text)') is not null and not exists(select 1 from (values(0::oid),('anon'::regrole::oid),('authenticated'::regrole::oid),('service_role'::regrole::oid))r(oid) where has_function_privilege(r.oid,to_regprocedure('public.complete_openingfit_mission_training_session(uuid,uuid,uuid,text)'),'execute'))"
$stage004D="to_regprocedure('public.project_openingfit_mission_activity(uuid)') is not null and to_regprocedure('public.assign_openingfit_mission_with_allowance(uuid,uuid,boolean,text)') is not null and not exists(select 1 from (values(0::oid),('anon'::regrole::oid),('authenticated'::regrole::oid),('service_role'::regrole::oid))r(oid) cross join(values(to_regprocedure('public.project_openingfit_mission_activity(uuid)')),(to_regprocedure('public.assign_openingfit_mission_with_allowance(uuid,uuid,boolean,text)')))f(oid) where has_function_privilege(r.oid,f.oid,'execute'))"
$stage004E="public.openingfit_missions_schema_readiness()->>'schemaVersion'='4' and public.openingfit_missions_schema_readiness()->>'activityProjectorReady'='true' and (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname=any(array['record_openingfit_mission_event','complete_openingfit_mission_training_session','project_openingfit_mission_activity','project_openingfit_mission_session_activity','assign_openingfit_mission_with_allowance','openingfit_missions_operator_diagnostics','openingfit_missions_schema_readiness']))=7 and not exists(select 1 from pg_proc p where p.oid=any(array[to_regprocedure('public.record_openingfit_mission_event(uuid,uuid,text,text,jsonb)'),to_regprocedure('public.complete_openingfit_mission_training_session(uuid,uuid,uuid,text)'),to_regprocedure('public.project_openingfit_mission_activity(uuid)'),to_regprocedure('public.project_openingfit_mission_session_activity(uuid,uuid)'),to_regprocedure('public.assign_openingfit_mission_with_allowance(uuid,uuid,boolean,text)'),to_regprocedure('public.openingfit_missions_operator_diagnostics(integer)'),to_regprocedure('public.openingfit_missions_schema_readiness()')]) and (not p.prosecdef or not p.proconfig @> array['search_path=public'])) and not exists(select 1 from (values(0::oid),('anon'::regrole::oid),('authenticated'::regrole::oid))r(oid) cross join(values(to_regprocedure('public.record_openingfit_mission_event(uuid,uuid,text,text,jsonb)')),(to_regprocedure('public.complete_openingfit_mission_training_session(uuid,uuid,uuid,text)')),(to_regprocedure('public.project_openingfit_mission_activity(uuid)')),(to_regprocedure('public.project_openingfit_mission_session_activity(uuid,uuid)')),(to_regprocedure('public.assign_openingfit_mission_with_allowance(uuid,uuid,boolean,text)')),(to_regprocedure('public.openingfit_missions_operator_diagnostics(integer)')),(to_regprocedure('public.openingfit_missions_schema_readiness()')))f(oid) where has_function_privilege(r.oid,f.oid,'execute')) and not exists(select 1 from (values(to_regprocedure('public.record_openingfit_mission_event(uuid,uuid,text,text,jsonb)')),(to_regprocedure('public.complete_openingfit_mission_training_session(uuid,uuid,uuid,text)')),(to_regprocedure('public.project_openingfit_mission_activity(uuid)')),(to_regprocedure('public.project_openingfit_mission_session_activity(uuid,uuid)')),(to_regprocedure('public.assign_openingfit_mission_with_allowance(uuid,uuid,boolean,text)')),(to_regprocedure('public.openingfit_missions_operator_diagnostics(integer)')),(to_regprocedure('public.openingfit_missions_schema_readiness()')))f(oid) where not has_function_privilege('service_role',f.oid,'execute'))"
$split004=@(
 @{Name='004a';From=0;To=49;Requires="if public.openingfit_missions_schema_readiness()->>'schemaVersion'<>'3' then raise exception 'completed migration 003 is required'; end if;";After='';Complete=$stage004A;Assert="if not ($stage004A) then raise exception '004A postcondition failed'; end if;"},
 @{Name='004b';From=50;To=69;Requires="if not ($stage004A) then raise exception '004A is required'; end if;";After="revoke all on function public.record_openingfit_mission_event(uuid,uuid,text,text,jsonb) from public,anon,authenticated,service_role;`n";Complete="$stage004A and $stage004B";Assert="if not ($stage004B) then raise exception '004B postcondition failed'; end if;"},
 @{Name='004c';From=70;To=95;Requires="if not ($stage004A and $stage004B) then raise exception 'contained 004B is required'; end if;";After="revoke all on function public.complete_openingfit_mission_training_session(uuid,uuid,uuid,text) from public,anon,authenticated,service_role;`n";Complete="$stage004A and $stage004B and $stage004C";Assert="if not ($stage004C) then raise exception '004C postcondition failed'; end if;"},
 @{Name='004d';From=96;To=135;Requires="if not ($stage004A and $stage004B and $stage004C) then raise exception 'contained 004C is required'; end if;";After="revoke all on function public.project_openingfit_mission_activity(uuid),public.assign_openingfit_mission_with_allowance(uuid,uuid,boolean,text) from public,anon,authenticated,service_role;`n";Complete="$stage004A and $stage004B and $stage004C and $stage004D";Assert="if not ($stage004D) then raise exception '004D postcondition failed'; end if;"},
 @{Name='004e';From=136;To=168;Requires="if not ($stage004D) then raise exception '004D required'; end if;";After="grant execute on function public.complete_openingfit_mission_training_session(uuid,uuid,uuid,text) to service_role;`n";Complete="$stage004A and $stage004E";Assert="if not ($stage004E) then raise exception '004E postcondition failed'; end if;"}
)
foreach($stage in $split004){
 $body=($lines004[$stage.From..$stage.To]-join "`n")+"`n"
 $header="-- $project $($stage.Name.ToUpper()); Missions disabled.`nBEGIN;`nDO `$precondition`$ begin $($stage.Requires) end `$precondition`$;`n-- SOURCE MIGRATION 004 STAGE BEGIN`n"
 $tail="-- SOURCE MIGRATION 004 STAGE END`n$($stage.After)DO `$assert`$ begin $($stage.Assert) end `$assert`$;`nCOMMIT;"
 Write-Utf8 (Join-Path $out "openingfit-missions-production-$($stage.Name)-execute.sql") ($header+$body+$tail)
 $verify=@"
-- READ ONLY. Migration $($stage.Name.ToUpper()) verification.
select case when to_regclass('public.openingfit_mission_events') is null then 'stage_absent' when ($($stage.Complete)) then 'stage_complete' else 'stage_partial' end classification;
"@
 Write-Utf8 (Join-Path $out "openingfit-missions-production-$($stage.Name)-verification.sql") $verify
}
...
$v4 += "`n-- 8. Fail-closed classification.`nselect case when to_regclass('public.openingfit_mission_events') is null then 'stage_absent' when ($stage004A and $stage004E) then 'stage_complete' else 'stage_partial' end classification;"
...
For migration 004, never paste the complete wrapper. Use checksum-approved SQL Editor stages 004A through 004E in order. Run the matching read-only verification after each stage and require `stage_complete`; obtain separate approval for every stage. The first authorized production write is 004A.
