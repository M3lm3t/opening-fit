...
test("Overview keeps one evidence renderer, compact mobile rules, and canonical CTA IDs", async () => {
  ...
  assert.match(styles, /\.primaryReportSummary \{ min-width: 0;/);
  assert.match(styles, /\.primaryReportCommand \{[^}]*overflow: hidden;/);
  assert.match(styles, /\.primaryReportCommand h3 \{[^}]*overflow-wrap: anywhere;/);
  assert.match(styles, /@media \(max-width: 520px\)[\s\S]*\.primaryReportCommand button,[\s\S]*\.primaryReportMore button \{ width: 100%; \}/);
});
