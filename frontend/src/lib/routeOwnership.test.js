...
test("canonical direct routes have one owner and explicit hydration policy", () => {
  ...
  assert.deepEqual(resolveOwnedProductRoute("/progress"), { view: "account", kind: "legacy-progress", hydrateReport: true });
  ...
  assert.equal(APP_NAV_ROUTES.home.path, "/dashboard");
});
