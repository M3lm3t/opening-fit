...
export function buildTrainingQueue(report = {}, repertoire = null, outcomes = []) {
  const habits = array(report.recurringOpeningHabits || report.recurring_opening_habits).filter((row) => row.habitType !== "GOOD_HABIT").slice(0, 3).map((row, index) => ({ id: `habit:${row.habitId || index}`, priority: 1500 + Number(row.occurrenceCount || 0) * 100 - index, type: row.habitType === "MIXED" ? "Mixed opening decision" : "Recurring opening habit", title: name(row), reason: row.habitType === "RECURRING_MISTAKE" ? `${row.playedMove} repeatedly leaves this position worse.` : "Review this repeated opening decision.", target: { ...row, opportunityId: row.habitId, openingId: row.canonicalOpeningId, gameId: row.gameReferences?.[0], issueType: String(row.habitType || "habit").toLowerCase(), reviewType: row.recommendedMove ? "move_review" : "concept_review", recurrenceCount: row.occurrenceCount } }));
  ...
  return [...habits, ...opportunities, ...weak, ...mistakes, ...selected].map((task) => failedIds.has(task.id) ? { ...task, priority: task.priority + 200, type: "Repeat a failed line" } : task).sort((a, b) => b.priority - a.priority).filter((task, index, rows) => rows.findIndex((row) => row.id === task.id) === index).slice(0, 4);
}
