...
test("renders meaningful improvement with previous and current values", () => {
  ...
  assert.ok(view.details.some((item) => item.title === "Repertoire Health" && /60 before · 68 now/.test(item.text)));
});
...
test("renders decline explicitly", () => {
  ...
  assert.ok(view.details.some((item) => item.title === "Repertoire Health" && item.statusLabel === "Declined with supporting evidence"));
});
...
test("small samples render insufficient data and never claim improvement", () => {
  ...
  assert.equal(view.details.find((item) => item.title === "Repertoire Health")?.statusLabel, "Not enough comparable evidence");
