...
BEGIN;
DO $precondition$ begin if not exists(select 1 from pg_trigger where tgrelid=to_regclass('public.openingfit_mission_training_sessions') and tgname='openingfit_protect_training_session' and not tgisinternal) then raise exception '003A is required'; end if; end $precondition$;
